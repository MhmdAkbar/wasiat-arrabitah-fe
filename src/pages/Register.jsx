    ...
    <div className="flex items-center justify-center min-h-screen bg-pattern relative z-0 py-10">
      ...
      <div className="bg-white p-10 rounded-2xl shadow-soft w-full max-w-md border-t-4 border-mosque-gold relative">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-mosque-dark">{t('auth.register_title')}</h1>
          <p className="text-sm text-gray-500 mt-1">{t('app_name')} - {t('mosque_name')}</p>
        </div>
        ...
        <div className="mt-6 text-center border-t border-gray-100 pt-5">
          <p className="text-sm text-gray-500">{t('auth.have_account')} <Link to="/" className="text-mosque-gold font-semibold hover:underline">{t('auth.login_btn')}</Link></p>
        </div>
      </div>
    </div>
  );
