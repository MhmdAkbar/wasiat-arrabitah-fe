...
export default function Login() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-pattern relative z-0">
      ...
      <div className="absolute top-0 left-0 w-full h-1/3 bg-gradient-to-b from-mosque-dark to-transparent -z-10 opacity-90"></div>
      ...
      <div className="bg-white p-10 rounded-2xl shadow-soft w-full max-w-md border-t-4 border-mosque-gold relative">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-mosque-dark tracking-wide">{t('app_name')}</h1>
          <p className="text-sm text-mosque-gold font-medium mt-1 uppercase tracking-widest">{t('mosque_name')}</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('auth.email')}</label>
            <input type="email" required placeholder="admin@arrabitah.id" value={email} onChange={(e) => setEmail(e.target.value)}
          ...
          <button disabled={loading} type="submit" className="w-full py-3 mt-2 rounded-lg font-semibold text-white bg-mosque-dark hover:bg-mosque-primary transition disabled:opacity-70">
            {loading ? t('auth.loading') : t('auth.login_btn')}
          </button>
        </form>
        
        <div className="mt-6 text-center border-t border-gray-100 pt-5">
          <p className="text-sm text-gray-500">{t('auth.no_account')} <Link to="/register" className="text-mosque-gold font-semibold hover:underline">{t('auth.register_btn')}</Link></p>
        </div>
      </div>
    </div>
  );
