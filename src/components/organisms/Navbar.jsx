...
export default function Navbar() {
  ...
  return (
    <header className="bg-white shadow-sm px-8 py-4 flex justify-between items-center sticky top-0 z-0">
      <div>
        <h3 className="text-xl font-semibold text-gray-800">{t('dashboard.welcome_title')}</h3>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm text-right hidden md:block">
            <div className="font-semibold text-gray-800">{name} <span className="text-[10px] bg-mosque-gold text-white px-1.5 py-0.5 rounded-sm uppercase tracking-wider ml-1">{role}</span></div> 
            <div className="text-xs text-gray-500">{email}</div>
          </div>
          <button onClick={handleLogout} className="ml-2 text-sm bg-red-50 text-red-600 px-3 py-2 rounded-lg hover:bg-red-100 transition font-medium flex items-center gap-2">
