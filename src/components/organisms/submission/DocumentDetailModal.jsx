...
export default function DocumentDetailModal({ isOpen, onClose, detailData, loading, cancelSubmission }) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden border-t-4 border-mosque-primary">
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="text-right">
                  <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${
                    ...
                    detailData.status === 'returned' ? 'bg-orange-100 text-orange-700' :
                  ...
                  {detailData.formTemplate.schemaDefinition.fields.map(field => (
                    <div key={field.key} className="col-span-1">
                      ...
                      <p className="text-sm text-gray-800 font-medium mt-1">
                        {field.type === 'number' 
                          ? new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumSignificantDigits: 3 }).format(detailData.formData[field.key])
        ...
        <div className="flex justify-between items-center p-4 border-t border-gray-100 bg-white flex-shrink-0">
          ...
          <div>
            {!loading && detailData && detailData.status === 'submitted' && (
              <button 
                type="button" 
            ...
            {/* Future "Resubmit" or "Edit" buttons can be added here for 'returned' status */}
          </div>

          {/* Right Side: Standard Close Button */}
          <button 
            type="button" 
            onClick={onClose} 
